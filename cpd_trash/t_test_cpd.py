...
class TTestCPDetector:
    ...
    def _check_for_changepoint(self) -> Tuple[float, bool]:
        current_std = np.sqrt(1.0/(self.current_t-1)*np.sum(np.power(np.array(self.current_obs)-self.current_mean,2)))
        test_statistic = (np.mean(np.array(self.current_obs)) - self.current_mean)/(current_std / (self.current_t**0.5))
        prob = float(self._get_prob(test_statistic))

        return prob, prob < self._p_limit

    def _get_prob(self, y) -> bool:
        p = stats.t.cdf(np.abs(y), self.current_t)
        prob = 2*(1 - p)
...
#     def _init_params(self) -> None:
#         self.current_mean = np.mean(np.array(self.current_obs))
#         self.current_std = np.sqrt(1.0/(self.current_t-1)*np.sum(np.power(np.array(self.current_obs)-self.current_mean,2)))
