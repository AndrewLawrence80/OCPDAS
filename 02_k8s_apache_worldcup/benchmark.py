...
class LoadGenerator:
    ...
    def run(self):
        ...
        # show the run time of a round of workload
...
if __name__ == "__main__":
    ...
    # scale the original request numbers to avoid request failure
    df["num_request"] = df["num_request"].apply(lambda x: x//10)
    workloads = df["num_request"].to_numpy()
    for workload in workloads:
        load_generator = LoadGenerator("http://192.168.49.2:30080", concurrency=100, interval=60, timeout=5, workloads=[workload], namespace="ocpdas", label="app=apache")
        load_generator.run()
