...
if __name__ == "__main__":
    df = pd.read_csv("dataset/workload_1998-07-04.csv")
    df["num_request"] = df["num_request"].apply(lambda x: x//10)
    workloads = df["num_request"].to_numpy()
    for workload in workloads:
        load_generator = LoadGenerator("http://192.168.49.2:30090", batch_size=100, interval=60, timeout=5, workloads=[workload], namespace="autoscaling", label="app=apache-10")
        load_generator.run()
